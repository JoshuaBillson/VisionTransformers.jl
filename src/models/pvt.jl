...
function PVT(embed_dims, depths, nheads; mlp_ratios=[8,8,4,4], sr_ratios=[8,4,2,1], qkv_bias=true, dropout=0.1, attn_dropout=0.1, drop_path=0.0, imsize=(224,224), inchannels=3, nclasses=1000)
    # Validate Arguments
...
function PVTStage(inplanes, outplanes, depth; nheads=8, imsize=(224,224), patchsize=(4,4), mlp_ratio=4, qkv_bias=false, dropout=0.0, attn_dropout=0.0, drop_path=0., sr_ratio=1)
    drop_path = drop_path isa Number ? repeat([drop_path], depth) : drop_path
    Flux.Chain(
        # Patch Embedding
        Flux.Chain(
            Flux.Conv(patchsize, inplanes=>outplanes; stride=patchsize), 
            Base.Fix2(permutedims, (3,1,2,4)),
            Flux.LayerNorm(outplanes),
            VariablePositionEmbedding(outplanes, imsize, patchsize; init=trunc_normal),
            Flux.Dropout(dropout), 
        ),
        [PVTBlock(outplanes, nheads; mlp_ratio, qkv_bias, sr_ratio, dropout=dropout, attn_dropout=attn_dropout, drop_path=drop_path[i]) for i in 1:depth]..., 
        Base.Fix2(permutedims, (2,3,1,4)),
    )
end

function PVTBlock(dim, nheads; mlp_ratio=4, qkv_bias=false, sr_ratio=1, dropout=0.0, attn_dropout=0.0, drop_path=0.)
    Flux.Chain(
        Flux.SkipConnection(
            Flux.Chain(
                Flux.LayerNorm(dim),
                SRAttention(dim; nheads, qkv_bias, sr_ratio=(sr_ratio,sr_ratio), attn_dropout_prob=attn_dropout, proj_dropout_prob=dropout),
